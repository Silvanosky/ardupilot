...
void i2c_init(i2c_t dev)
{
    ...
    _i2c_bus[dev].lock = xSemaphoreCreateMutex();
    ...
    gpio_reset_pin(_i2c_bus[dev].scl);
    gpio_reset_pin(_i2c_bus[dev].sda);
    ...
    gpio_config_t gpio_conf = {
        .pin_bit_mask = _i2c_bus[dev].scl_bit | _i2c_bus[dev].sda_bit,
...
int i2c_acquire(i2c_t dev)
{
    ...
    if (xSemaphoreTake(_i2c_bus[dev].lock, ( TickType_t ) 10) == pdTRUE)
        return 0;
    assert(false);
}
...
void i2c_release(i2c_t dev)
{
    ...
    xSemaphoreGive( _i2c_bus[dev].lock );
}
